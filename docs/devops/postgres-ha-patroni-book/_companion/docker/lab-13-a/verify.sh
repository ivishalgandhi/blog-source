#!/usr/bin/env bash
# verify.sh — LAB-13-A: Capture cluster state after a break scenario
...
if [ -n "$LEADER_NODE" ]; then
    ...
    docker compose exec -T "$LEADER_NODE" psql -U postgres -tc "SELECT client_addr, state, sent_lsn, write_lsn, flush_lsn, replay_lsn, pg_wal_lsn_diff(sent_lsn, replay_lsn) AS lag_bytes FROM pg_stat_replication;" 2>/dev/null || echo "  (no replication connections)"
else
